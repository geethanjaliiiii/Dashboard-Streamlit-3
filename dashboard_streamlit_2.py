...
if day_df.empty:
    ...
    # Theme-aware styles used only by the final performance section
    st.markdown(
        """
        <style>
    ...
    if not overall_df.empty:
        ...
        overall_mape_before = (
            (actual_all - before_all).abs() / actual_all
        ).mean() * 100

        overall_mape_after = (
            (actual_all - after_all).abs() / actual_all
        ).mean() * 100

        overall_mae_before = (
            actual_all - before_all
        ).abs().mean()

        overall_mae_after = (
            actual_all - after_all
        ).abs().mean()

        overall_rmse_before = np.sqrt(
            ((actual_all - before_all) ** 2).mean()
        )

        overall_rmse_after = np.sqrt(
            ((actual_all - after_all) ** 2).mean()
        )
        ...
        with right_summary:
            ...
            # Use only rows where 2-hour-ahead forecast exists
            ...
            if not two_hour_metrics_df.empty:
                ...
                twohr_forecast = two_hour_metrics_df[
                    "Two_Hour_Ahead_Forecast"
                ]
                ...
                mae_daily = (
                    actual_2hr - daily_2hr
                ).abs().mean()

                mae_2hr = (
                    actual_2hr - twohr_forecast
                ).abs().mean()
                ...
                rmse_2hr = np.sqrt(
                    ((actual_2hr - twohr_forecast) ** 2).mean()
                )
                ...
                st.warning(
                    "No valid 2-hour ahead forecast data available "
